...
class CnnStructModel(object):
    ...
    def __init__(self, config, input_):
        ...
        self._y_true = y_true = tf.placeholder(tf.float32, shape=[None], name='Labels')
        self.seq_lens = tf.placeholder(tf.int32, shape=[None], name='seq_lens')
        seq_input = tf.reshape(tf.slice(self._x, [0, 0, 0], [-1, -1, 4]), [-1, 4])
        struct_input = tf.slice(self._x, [0, 0, 4], [-1, -1, 5])

        self.structure_embedding = tf.get_variable("str_embedding", shape=[4, 10],
                            initializer=tf.contrib.layers.xavier_initializer())

        x = tf.reshape(tf.matmul(seq_input, self.structure_embedding), [-1, 41, 10])
        x = tf.concat([x, struct_input], axis=2)

        x_image = tf.expand_dims(x, 2)
        W_conv1 = tf.Variable(tf.random_normal([self.motif_len, 1, 15, self.num_motifs], stddev=0.01), name='W_Conv1')
        b_conv1 = tf.Variable(tf.constant(0.001, shape=[self.num_motifs]), name='b_conv1')
        ...
        cost_batch = tf.square(h_final - y_true)
        self._cost = cost = tf.reduce_mean(cost_batch)
        norm_w = (tf.reduce_sum(Wconv1 * W_conv1) + tf.reduce_sum(tf.abs(W_conv2 * W_conv2)
        optimizer = tf.train.AdamOptimizer(learning_rate=eta_model)
        self._train_op = optimizer.minimize(cost + norm_w * lam_model)
        self._predict_op = h_final
...
class RnnStructModel(object):
    ...
    def __init__(self, config, input_):
        ...
        self.seq_lens = tf.placeholder(tf.int32, shape=[None], name='seq_lens')
        x_image = tf.expand_dims(x, 2)
        W_conv1 = tf.get_variable("W_Conv1", shape=[self.motif_len, 1, 9, self.num_motifs],
                        initializer=tf.contrib.layers.xavier_initializer())
        b_conv1 = tf.Variable(tf.constant(0.001, shape=[self.num_motifs]), name='b_conv1')
        ...
        W_conv2 = tf.get_variable("W_Conv2", shape=[self.motif_len2, 1, self.num_motifs, 10],
                        initializer=tf.contrib.layers.xavier_initializer())

        b_conv2 = tf.Variable(tf.constant(0.001, shape=[10]), name='b_conv2')
        h_conv2 = tf.nn.conv2d(h_relu_conv1, W_conv2,
                               strides=[1, 1, 1, 1], padding='SAME')
        n_hidden = config.get('lstm_size', 20)
        ...
        cost_batch = tf.square(h_final - y_true)
        self._cost = cost = tf.reduce_mean(cost_batch, name='cost')
        norm_w = (tf.reduce_sum(tf.square(W_conv1)) + tf.reduce_sum(tf.square(W_conv2)) + tf.reduce_sum(tf.square(W_out)))
        optimizer = tf.train.AdamOptimizer(learning_rate=eta_model)
...
def load_data_rnac2013(protein_name):
    ...
    for line_struct in infile_struct:
        ...
        if (line_id in dic_ban) == False:
            if belong[line_id] == 1:
                structures_A.append(line_struct)
            else:
                structures_B.append(line_struct)
