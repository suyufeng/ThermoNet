...
class CnnStructModel_attention(object):
    ...
    def __init__(self, config, input_):
        ...
        self.motif_len = config['filter_lengths'][0]  # Tunable Motif length
        self.num_motifs = config['num_filters'][0]  # Number of tunable motifs
        self.motif_len2 = config['filter_lengths'][0]
        self.num_motifs2 = config['num_filters'][0]
        self.embedding_size = config['embedding_size']
        self.k_mer = int(config['k_mer'])
        attention_flag = config['attention_flag']
        GUI_flag = False
        Res_net = False
        layer_num = 5
        ...
        x_struct_image = tf.expand_dims(struct_input, 2)
        x_seq_image = tf.expand_dims(structure_result, 2)

        self.struct_conv = x_struct_image
        kernel_step = 0
        tmp = []
        for i in range(layer_num):
            if i % 2 == 1 and i != 1 and Res_net == True:
                self.struct_conv = (self.struct_conv + tmp) * math.sqrt(1./2)
            if i == 0:
                kernel_step = self.motif_len
            else:
                kernel_step = self.motif_len2
            if GUI_flag == False:
                self.struct_conv = tf.layers.conv2d(self.struct_conv, self.num_motifs, (kernel_step, 1), padding='same',
                                             kernel_initializer=tf.contrib.layers.xavier_initializer())
                self.struct_conv = tf.layers.batch_normalization(self.struct_conv)
                self.struct_conv = tf.nn.relu(self.struct_conv)
                # self.struct_conv = tf.nn.dropout(self.struct_conv, keep_prob=0.9)
            else:
                self.struct_conv = tf.layers.conv2d(self.struct_conv, self.num_motifs * 2, (kernel_step, 1), padding='same',
                                             kernel_initializer=tf.contrib.layers.xavier_initializer())
                self.struct_conv = tf.layers.batch_normalization(self.struct_conv)
                self.struct_conv1 = tf.nn.tanh(tf.slice(self.struct_conv, [0, 0, 0, 0], [-1, -1, -1, self.num_motifs]))
                self.struct_conv2 = tf.slice(self.struct_conv, [0, 0, 0, self.num_motifs], [-1, -1, -1, self.num_motifs])
                self.struct_conv = self.struct_conv1 * self.struct_conv2
            if Res_net == True and i % 2 == 1:
                tmp = self.struct_conv

        self.seq_conv = x_seq_image
        kernel_step = 0
        tmp = []
        for i in range(layer_num):
            if i % 2 == 1 and i != 1 and Res_net == True:
                self.seq_conv = (self.seq_conv + tmp) * math.sqrt(1. / 2)
            if i == 0:
                kernel_step = self.motif_len
            else:
                kernel_step = self.motif_len2
            if GUI_flag == False:
                self.seq_conv = tf.layers.conv2d(self.seq_conv, self.num_motifs, (kernel_step, 1), padding='same',
                                                    kernel_initializer=tf.contrib.layers.xavier_initializer())
                self.seq_conv = tf.layers.batch_normalization(self.seq_conv)
                self.seq_conv = tf.nn.relu(self.seq_conv)
                # self.seq_conv = tf.nn.dropout(self.seq_conv, keep_prob=0.9)
            else:
                self.seq_conv = tf.layers.conv2d(self.seq_conv, self.num_motifs * 2, (kernel_step, 1), padding='same',
                                             kernel_initializer=tf.contrib.layers.xavier_initializer())
                self.seq_conv = tf.layers.batch_normalization(self.seq_conv)
                self.seq_conv1 = tf.nn.tanh(tf.slice(self.seq_conv, [0, 0, 0, 0], [-1, -1, -1, self.num_motifs]))
                self.seq_conv2 = tf.slice(self.seq_conv, [0, 0, 0, self.num_motifs],
                                             [-1, -1, -1, self.num_motifs])
                self.seq_conv = self.seq_conv1 * self.seq_conv2
            if Res_net == True and i % 2 == 1:
                tmp = self.seq_conv

        self.final_output = tf.concat([self.seq_conv, self.struct_conv], axis = 3)
        self.num_motifs *= 2
        self.conv2 = tf.reshape(tf.squeeze(self.final_output, axis=[2], name='lstm_input'), [-1, self.num_motifs])

        if(attention_flag):
            self.attention = tf.get_variable("Attention", shape=[self.num_motifs, 1],
                                             initializer=tf.contrib.layers.xavier_initializer())

            self.attention = tf.matmul(self.conv2, self.attention) / math.sqrt(self.num_motifs)
            self.attention = tf.nn.softmax(self.attention)
            self.final = tf.matmul(tf.reshape(self.attention, [-1, 1, 41]),
                                   tf.reshape(self.conv2, [-1, 41, self.num_motifs]))
            self.final = tf.squeeze(self.final, axis=[1])
            h_final = tf.reduce_mean(self.final, axis=1)
        else:
            self.conv2 = tf.reshape(self.conv2, [-1, 41 * self.num_motifs])
            self.conv2 = tf.nn.dropout(self.conv2, keep_prob=0.9)
            h_final = tf.squeeze(
                tf.layers.dense(self.conv2, 1, kernel_initializer=tf.truncated_normal_initializer(0.001)))
            #h_final = tf.squeeze(tf.layers.dense(self.conv2, 1, kernel_initializer=tf.contrib.layers.xavier_initializer()))
            #h_final = tf.squeeze(tf.layers.dense(self.conv2, 1, kernel_initializer=tf.contrib.layers.xavier_initializer()))
        cost_batch = tf.square(h_final - y_true)
        self._cost = cost = tf.reduce_mean(cost_batch)
        # norm_w = (tf.reduce_sum(tf.abs(W_conv1)) + tf.reduce_sum(tf.abs(W_conv2)))
...
def run_epoch_parallel(session, models, input_data, config, epoch, train=False, verbose=False, testing=False,
    ...
    print("HERE", cost_temp[0], Nbatch_train)
    cost_train = cost_temp / float(Nbatch_train)
    for j in range(num_models):
        if isinstance(input_data, list):
    ...
    if testing or scores:
        ...
        if isinstance(input_data,list):
            ...
            output = test_scores
            print(np.mean(output), np.min(output), np.max(output))
...
def load_data_rnac2013(protein_name):
    ...
    infile_struct = np.load('../data/rnac/result.txt.npy').reshape([-1, 5, 41])

    import scipy.sparse

    # t = scipy.sparse.load_npz("../data/rnac/contact_map.txt.npz")
    # feature_map_input = np.array(t.toarray())
    # infile_struct = np.reshape(feature_map_input, [-1, 100, 5, 41]).astype(np.float64)
    ...
    print(data_one_hot_test.shape)
    print(structures_test.shape)
    save_target = "../data/rnac/npz_archives_kmer/" +protein_name
...
def load_data_rnac2009(protein_name):
    ...
    training_cases = data_one_hot_training.shape[0]
    test_cases = data_one_hot_test.shape[0]
    save_target = os.path.join('data/rnac_2009/npz_archives_kmer/', protein_name + '.npz')
    np.savez(save_target, data_one_hot_training=data_one_hot_training,
             labels_training=labels_training,
             data_one_hot_test=data_one_hot_test,
             labels_test=labels_test, training_cases=training_cases,
...
def load_data_clipseq(protein_name):
    ...
    save_target = os.path.join('data/GraphProt_CLIP_sequences/npz_archives_kmer', protein_name + '.npz')
...
def load_data_rnac_s(path):
    ...
    if not (os.path.exists(os.path.join(path, 'npz_archives_kmer'))):
        os.makedirs(os.path.join(path, 'npz_archives_kmer'))
    save_path = os.path.join(path, 'npz_archives_kmer', 'SLBP_rnacs.npz')
...
def load_data(protein_name):
    if 'RNCMPT' in protein_name:
        if not (os.path.isfile('../data/rnac/npz_archives_kmer/' + str(protein_name) + '.npz')):
            print("[!] Processing input for " + protein_name)
            load_data_rnac2013(protein_name)
        return np.load('../data/rnac/npz_archives_kmer/' + str(protein_name) + '.npz')
    elif protein_name == 'SLBP_rnacs':
        if not (os.path.isfile('../data/rnac_s/npz_archives_kmer/' + str(protein_name)+'.npz')):
            print("[!] Processing input for " + protein_name)
            load_data_rnac_s('../data/rnac_s')
        return np.load(('../data/rnac_s/npz_archives_kmer/' + str(protein_name)+'.npz'))
    else:
        if not (os.path.isfile('../data/rnac_2009/npz_archives_kmer/' + str(protein_name) + '.npz')):
            print("[!] Processing input for " + protein_name)
            load_data_rnac2009(protein_name)
        return np.load('../data/rnac_2009/npz_archives_kmer/' + str(protein_name) + '.npz')


def generate_configs_CNN(num_calibrations, config):
    configs = []
    ...
    #                 for filter_num in [8]:
                    ...
                    num_filters = [filter_num * (i + 1) for i in range(num_conv_layers)]
                    init_scale = 0.00001
...
def generate_configs(num_calibrations, model_type, config):
    if model_type=='CNN':
        return generate_configs_CNN(num_calibrations)
    if model_type=='CNN_struct':
        return generate_configs_CNN(num_calibrations, config)
    if model_type=='CNN_attention':
        return generate_configs_CNN(num_calibrations, config)
    if model_type=='RNN_struct':
        return generate_configs_RNN(num_calibrations, config)
    if model_type == 'RNN':
