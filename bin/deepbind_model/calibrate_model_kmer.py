...
def calibrate_model(train_config):
    ...
    input_configuration = utils.input_config('big')
    configs = utils.generate_configs(num_calibrations, model_type, train_config)
    # print(configs)
    inputs = []
    ...
    num_round = ((len(configs) - max_per_round + 1) // max_per_round) + 1
                ...
                with tf.Session(config=config) as session:
                    ...
                    (val_cost[fold, t * max_per_round: upper_bound, :],
                     val_pearson[fold, t * max_per_round: upper_bound, :], _, _) = \
                        utils.train_model_parallel(session, train_config, models, inputs[fold], epochs=epochs,
                                                   early_stop=False)
